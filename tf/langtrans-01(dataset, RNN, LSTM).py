...
from keras.api.layers import Dense, Embedding, Input, LSTM, Bidirectional, Concatenate
...
einputs = pad_sequences(eng_sequences, maxlen=max_len_eng, padding="pre")
print("encoder input shape :", einputs.shape)
print("encoder_inputs[0] : ", einputs[1500])

dinputs = pad_sequences(kor_input_sequences, maxlen=max_len_kor, padding="post")
print("\ndecoder input shape :", dinputs.shape)
print("decoder_inputs[0] : ", dinputs[1500])

dtargets = pad_sequences(kor_target_sequences, maxlen=max_len_kor, padding="post")
...
embedding_matrix = make_embedding(
    num_words_eng, EMBEDDING_DIM, tokenizer_eng, MAX_VOCAB_SIZE
)
# %%
...
embedding_layer = Embedding(
    num_words_eng, EMBEDDING_DIM, weights=[embedding_matrix], trainable=True
)
...
model_teacher_forcing.compile(
    loss="sparse_categorical_crossentropy", optimizer=Adam(0.001), metrics=["accuracy"]
)
...
checkpoint = ModelCheckpoint(
    "langtrans-01(dataset, RNN, LSTM).keras",
    save_best_only=True,
    monitor="val_loss",
    mode="min",
)
...
dataset_slice = slice(0, 3000)
einputs_subset = einputs[dataset_slice]
dinputs_subset = dinputs[dataset_slice]
dtargets_subset = dtargets[dataset_slice]

dataset_size = len(einputs_subset)
dataset = tf.data.Dataset.from_tensor_slices(
    ((einputs_subset, dinputs_subset), dtargets_subset)
)
